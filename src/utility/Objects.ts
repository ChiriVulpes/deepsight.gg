namespace Objects {
	...
	export function map<T, R extends [Key, any]> (object: T, mapper: (pair: KeyValuePairs<T>) => R) {
		...
		return Object.fromEntries(Object.entries(object).map(mapper as any)) as { [KEY in R[0]]: R[1] };
	}

	export async function mapAsync<T, R extends [Key, any]> (object: T, mapper: (pair: KeyValuePairs<T>) => Promise<R>) {
		// eslint-disable-next-line @typescript-eslint/no-unsafe-argument
		return Object.fromEntries(await Promise.all(Object.entries(object).map(mapper as any)) as any) as { [KEY in R[0]]: R[1] };
	}
}
