	...
	const JSONRender = Component((component, value?: unknown, links?: State<Links | undefined>) => {
		...
		const JSONLinks = Component((component, key: string | number, path?: (string | number)[], leadingSpace = ' ') => component.and(DisplaySlot).use(links, (slot, links) => {
			...
			if ('enum' in link) {
				...
				const defs = links.definitions?.[link.component] as Partial<Record<string | number, object>> | undefined
				const linkedDef = defs?.[key]
				DataProvider.SINGLE.prep(link.component, key)
				if (linkedDef) ref
					.style('data-overlay-json-reference-definition')
					.append(Link(`/data/${link.component}/${key}`)
